...
public class MultiThreadedConstellation {
    ...
    public synchronized ActivityIdentifier performSubmit(Activity a) {
        ...
        if (logger.isInfoEnabled()) {
            logger.info(
                    "No local executor for this activity (no identifier yet)");
        }
        ...
        int i = next++;
        next = next % workerCount;
        return workers[i].performSubmit(a);
    ...
    private ConstellationIdentifierImpl deliverLocally(
            ConstellationIdentifierImpl cid, EventMessage m) {
    ...
    public boolean handleStealReply(SingleThreadedConstellation src,
            StealReply m) {
    ...
    public ActivityRecord[] handleStealRequest(
            final SingleThreadedConstellation c, final int stealSize) {
        ...
        for (int i = 0; i < workerCount; i++) {
            ...
            final SingleThreadedConstellation tmp = workers[(rnd + i)
                    % workerCount];

            if (tmp != c && poolMatrix[rank][tmp.getRank()]) {

                final int size = tmp.attemptSteal(result, context,
        ...
        final StealRequest sr = new StealRequest(c.identifier(), context,
    ...
    public synchronized void register(
            SingleThreadedConstellation constellation) {
